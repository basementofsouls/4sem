...
void main() {
	...
	try
	{
		...
		for (int i = 1; i <= MESSAGENUM; i++) {
			...
			if (sendto(cS, buffer.c_str(), buffer.size() + 1, NULL, (SOCKADDR*)&serv, servSize) == SOCKET_ERROR) {
				throw  SetErrorMsgText("Cleanup:", WSAGetLastError());
			}
		
			cout << "Клиент получил: ";
			if (recvfrom(cS, cbuff, SIZE, NULL, (sockaddr*)&serv, &servSize) == SOCKET_ERROR) {
				throw  SetErrorMsgText("Cleanup:", WSAGetLastError());
			}

			cout << cbuff << endl;
		}
		for(__int8 i = 0; i < 100; i++)
		if (sendto(cS, endmessage, 1, NULL, (SOCKADDR*)&serv, servSize) == SOCKET_ERROR) {
			throw  SetErrorMsgText("Cleanup:", WSAGetLastError());
		}
